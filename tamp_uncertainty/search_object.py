...
def pick_execute_fn(a, b, s, store):
    ...
    if s is None: # effect fn
        ...
        desired_orientation_pick=torch.tensor([store.get(a.args[1])]*env.unwrapped.num_envs, dtype=torch.float32,device=env.unwrapped.device) # grasp
        while True:
            dones = env.step(actions)[-2]
...
def drop_execute_fn(a, b, s, store):
    ...
    if s is None: # effect fn
        ...
        count = 0
        desired_orientation_drop=torch.tensor([store.get(a.args[1])]*env.unwrapped.num_envs, dtype=torch.float32, device=env.unwrapped.device)
        while True:
            dones = env.step(actions)[-2]
